...
| DSH 原生工具 schema | `GenerateOptions.tools` | 可能与 AGY 工具重复 | 立即返回 `UNSUPPORTED_TOOLS` | 无，避免双 loop |
...
2. DSH 传入非空 `tools` 时，在启动 AGY 前失败，避免工具被执行两次。
3. 检测到权限请求时终止 AGY 子进程，避免 headless 进程无限等待。
4. AGY 工具事件只作为外部数据解析，不信任其中的路径、命令或参数。
5. 工具桥接要等 M6 后续重新设计，不能通过“把 AGY 工具名伪装成 DSH tool-call”实现。
