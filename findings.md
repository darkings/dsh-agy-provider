...
- GitHub Actions 第二次只剩 Node 20 测试入口差异：Node 20 不展开 `node --test tests/*.test.mjs` 的 glob。已改为 `scripts/run-tests.mjs` 动态枚举 `.test.mjs` 文件；第三轮 CI run `32112054709` 的 Node 20/24 两个 job 均已通过。
