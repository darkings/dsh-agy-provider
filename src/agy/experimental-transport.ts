...
export interface PersistentIncomingFrame {
  readonly kind: 'ready' | 'event' | 'complete' | 'error'
  readonly requestId?: string
  readonly sessionId?: string
  readonly payload?: unknown
...
class PersistentAgyWorker {
  ...
  async request(input: PersistentTransportRequest): Promise<PersistentTransportResult> {
    ...
    return new Promise<PersistentTransportResult>((resolve, reject) => {
      ...
      void this.sendRequest(active, input.payload)
    })
  }
  ...
  async stop(reason: 'dispose' | 'idle' | 'reset' = 'dispose'): Promise<void> {
    ...
    if (reason === 'dispose' || reason === 'idle') {
      try {
        child.stdin.end(encodePersistentFrame({ kind: 'shutdown' }, this.options.maxFrameBytes))
      } catch {
  ...
  private handleLine(line: string): void {
    let frame: PersistentIncomingFrame
    try {
      frame = parsePersistentFrame(line)
    } catch (error) {
    ...
    if (frame.kind === 'ready') {
      if (frame.sessionId !== undefined && frame.sessionId !== this.sessionId) {
        this.rejectReady(new PersistentTransportError(
          'Persistent AGY worker ready frame belongs to another session',
          'PROTOCOL_ERROR',
        ))
        this.failActiveAndReset(new PersistentTransportError(
          'Persistent AGY worker session ownership check failed',
          'PROTOCOL_ERROR',
        ))
        return
      }
    ...
    if (active === undefined) {
      this.failActiveAndReset(new PersistentTransportError(
        'Persistent AGY worker emitted a response without an active request',
        'PROTOCOL_ERROR',
      ))
      return
    }
    ...
    if (frame.requestId !== active.requestId || frame.sessionId !== this.sessionId) {
      this.failActiveAndReset(new PersistentTransportError(
        'Persistent AGY worker response correlation failed',
        'PROTOCOL_ERROR',
      ))
      return
    }
    if (frame.kind === 'event') {
      ...
      active.events.push(frame.payload)
      try {
        active.onEvent?.(frame.payload)
      } catch (error) {
        this.failActiveAndReset(new PersistentTransportError(
    ...
    if (frame.kind === 'error') {
      this.failActiveAndReset(new PersistentTransportError(
        'Persistent AGY worker returned an error frame',
        'WORKER_ERROR',
      ))
      return
    }
  ...
  private async sendRequest(active: ActiveRequest, payload: unknown): Promise<void> {
    try {
      const frame = encodePersistentFrame({
        kind: 'request',
